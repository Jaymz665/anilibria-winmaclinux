...
int ApplicationSettings::normalX() noexcept
{
    return m_Settings->value("normalx", 0).toInt();
}

void ApplicationSettings::setNormalX(int normalX) noexcept
{
    m_Settings->setValue("normalx", normalX);

    emit normalXChanged();
}

int ApplicationSettings::normalY() noexcept
{
    return m_Settings->value("normaly", 0).toInt();
}

void ApplicationSettings::setNormalY(int normalY) noexcept
{
    m_Settings->setValue("normaly", normalY);

    emit normalYChanged();
}

int ApplicationSettings::normalWidth() noexcept
{
    return m_Settings->value("normalwidth", 0).toInt();
}

void ApplicationSettings::setNormalWidth(int normalWidth) noexcept
{
    m_Settings->setValue("normalwidth", normalWidth);

    emit normalWidthChanged();
}

int ApplicationSettings::normalHeight() noexcept
{
    return m_Settings->value("normalheight", 0).toInt();
}

void ApplicationSettings::setNormalHeight(int normalHeight) noexcept
{
